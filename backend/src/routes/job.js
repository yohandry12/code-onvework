...
module.exports = function (io) {
  ...
  // =============================================================
  // --- GET /api/candidates/recommendations - Toutes les recommandations par candidat ---
  // =============================================================
  router.get("/candidates/recommendations", async (req, res) => {
    try {
      ...
      // Récupérer tous les utilisateurs candidats avec leurs profils
      ...
      // Pour chaque candidat, récupérer les recommandations paginées
      const enrichedCandidates = await Promise.all(
        candidates.map(async (candidate) => {
          const candidateId = candidate.id;

          // Compter le total des recommandations
          ...
          // Récupérer les recommandations paginées (première page pour le résumé)
          ...
          const avgRatingResult = await Recommendation.findAll({
            where: { employeeId: candidateId },
          ...
          const transformedRecommendations = recommendations.map((rec) => ({
            ...
            employerName: `${rec.employer.firstName} ${rec.employer.lastName}`,
          ...
          return {
            candidateId,
            candidateName: `${candidate.candidateProfile.firstName} ${candidate.candidateProfile.lastName}`,
            avatarUrl: candidate.candidateProfile.avatarUrl || null,
            email: candidate.email,
            totalRecommendations,
            averageRating,
            badge: candidate.candidateProfile.recommendationBadge,
      ...
      // Filtrer les candidats null et trier par nombre de recommandations
      const candidatesWithRecommendations = enrichedCandidates
        .filter((c) => c !== null)
        .sort((a, b) => b.totalRecommendations - a.totalRecommendations);

      // Résumé global
      const totalRecommendationsAcrossAll =
        candidatesWithRecommendations.reduce(
      ...
      res.json({
        ...
        summary: {
          totalCandidates: candidatesWithRecommendations.length,
          totalRecommendationsAcrossAll,
        },
      });
    } catch (error) {
  ...
  // =============================================================
  // --- GET /api/candidates/:candidateId/recommendations - Recommandations d'un candidat ---
  // =============================================================
  router.get("/candidates/:candidateId/recommendations", async (req, res) => {
    try {
      ...
      const candidateUser = await User.findByPk(candidateId, {
        include: [
          {
            model: CandidateProfile,
            as: "candidateProfile",
      ...
      res.json({
        ...
        data: {
          ...
          avatarUrl: candidateProfile.avatarUrl || null,
          email: candidateUser.email,
          totalRecommendations,
          averageRating,
          badge: candidateProfile.recommendationBadge,
          recommendations: transformedRecommendations,
          pagination: {
            currentPage: pageNum,
            totalPages: totalPages > 0 ? totalPages : 1,
      ...
      logger.error("Erreur récupération recommandations candidat:", error);
      res.status(500).json({ success: false, error: "Erreur serveur." });
    }
  });
