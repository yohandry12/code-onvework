    ...
    async (req, res) => {
      try {
        ...
        if (existingTestimonial) {
          return res.status(400).json({
            success: false,
            error: "Vous avez déjà soumis un témoignage.",
          });
        }
        ...
        res.status(500).json({
          success: false,
          error: "Erreur lors de la soumission de votre témoignage.",
        });
      }
    }
  );
  ...
  router.get("/featured", async (req, res) => {
    try {
      ...
      const testimonials = await Testimonial.findAll({
        ...
        limit: 6,
        include: {
          // C'est ici qu'on fait la "jointure" (le populate)
          model: User,
          as: "author", // L'alias défini dans models/index.js
          attributes: ["id", "role"], // On sélectionne les champs de User...
          include: [
            // ...et on imbrique une autre jointure pour le profil spécifique
            {
      ...
      const formattedTestimonials = testimonials.map((t) => {
        ...
        if (plain.author) {
          plain.author.profile =
            plain.author.candidateProfile || plain.author.clientProfile;
  ...
  router.get("/", authenticateToken, requireRole("admin"), async (req, res) => {
    try {
      const allTestimonials = await Testimonial.findAll({
        order: [["createdAt", "DESC"]],
        include: {
          // Jointure simple pour avoir le nom de l'auteur
          model: User,
          as: "author",
    ...
    async (req, res) => {
      try {
        ...
        if (!testimonial) {
          return res
            .status(404)
            .json({ success: false, error: "Témoignage non trouvé." });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!testimonial) {
          return res
            .status(404)
            .json({ success: false, error: "Témoignage introuvable." });
        }

        await testimonial.destroy();
        logger.info(`Témoignage ${id} supprimé par l'admin ${req.user.id}`);
        ...
        logger.error("Erreur suppression témoignage:", error);
        res.status(500).json({ success: false, error: "Erreur serveur." });
      }
    }
  );
